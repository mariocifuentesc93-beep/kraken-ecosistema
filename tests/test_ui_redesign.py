...
class UiRedesignSmokeTests(unittest.TestCase):
    ...
    def test_empty_dashboard_at_1366_and_1600(self):
        ...
        for width,height in ((1366,768),(1600,900)):
            window.resize(width,height); window.show(); self.app.processEvents(); self.assertFalse(window.grab().isNull()); self.assertGreaterEqual(window.menu.width(),170); self.assertGreaterEqual(window.menu.count(),18)

...
class AnalyticsPage(QWidget):
    ...
    def build_ui(self):
        ...
        self.tables=QTableWidget(); self.tables.setColumnCount(5); self.tables.setHorizontalHeaderLabels(["Grupo","Trades","Neto","Win rate","Tipo"]); self.tables.setStyleSheet(table_style()); self.tables.setMaximumHeight(210); layout.addWidget(self.tables)
        chart_titles=("Balance curve","Equity curve","Drawdown curve","Daily P/L","Monthly P/L","Win/loss distribution","Results by symbol","Results by weekday","Results by hour","TP1/TP2/TP3/SL")
    ...
    def refresh(self):
        ...
        metrics=trading_analytics_service.metrics(self.filters()); series=trading_analytics_service.series(self.filters()); labels=(("Net P/L","net"),("Gross profit","gross_profit"),("Gross loss","gross_loss"),("Win rate","win_rate"),("Profit factor","profit_factor"),("Expectancy","expectancy"),("Total trades","total"),("Average trade","average_trade"),("Maximum drawdown","maximum_drawdown"),("Current streak","current_streak"),("Best streak","best_streak"),("Average duration","average_duration"))
        while self.cards.count(): item=self.cards.takeAt(0); item.widget().deleteLater() if item.widget() else None
        for index,(label,key) in enumerate(labels): card=QLabel(f"{label}\n{metrics[key]}"); card.setAlignment(Qt.AlignCenter); card.setStyleSheet(f"background:{CARD_COLOR};border:1px solid #3B4252;border-radius:6px;padding:8px;color:white;"); self.cards.addWidget(card,index//4,index%4)
        curve=dict((str(i),point[1]) for i,point in enumerate(metrics["curve"])); drawdown=dict((str(i),-point[2]) for i,point in enumerate(metrics["curve"])); mapping={"Balance curve":curve,"Equity curve":curve,"Drawdown curve":drawdown,"Daily P/L":series["daily"],"Monthly P/L":series["monthly"],"Win/loss distribution":{"Wins":metrics["gross_profit"],"Losses":metrics["gross_loss"]},"Results by symbol":series["symbol"],"Results by weekday":series["weekday"],"Results by hour":series["hour"],"TP1/TP2/TP3/SL":series["result"]}
